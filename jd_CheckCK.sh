...
verify_ck(){
    ...
    for i in ${!value[@]}; do
        ...
        [[ "$NOTIFY_SHOWNAMETYPE" ]] && full_name[i]="【${ori_sn[i]}】${pt_pin[i]}" || full_name[i]="full_name[i]"
        ...
        if [[ $CK_WxPusherUid = 1 || $CK_WxPusherUid = 2 ]]; then
            ...
            if [[ $(echo ${remarks[i]} | grep -Eo "UID_[^@]+") ]]; then
                Uid[i]="$(echo ${remarks[i]} | grep -Eo "UID_[^@]+")"
            elif [[ -f $CK_WxPusherUid_dir/CK_WxPusherUid.json ]]; then
                Uid[i]="$(def_json_value "$CK_WxPusherUid_dir/CK_WxPusherUid.json" Uid $((i+1)))"
            else                
                Uid[i]=""
            fi
            tmp17=" {\n\t\"序号\": \"${ori_sn[i]}\",\n\t\"pin\": \"${pin[i]}\",\n\t\"备注\": \"${remarks[i]}\",\n\t\"pt_pin\": \"${pt_pin[i]}\",\n\t\"Uid\": \"${Uid[i]}\"\n }"
